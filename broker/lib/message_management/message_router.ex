defmodule MessageRouter do
  ...
  def publish_message_to_all(subscription_list, message) do
    for {client, lst} <- subscription_list do
      if(Enum.member?(lst, Map.get(message, "topic")) || Enum.member?(lst, Map.get(message, "producer"))) do
  ...
  def verify_message(message) do
    if(Map.get(message, "producer")!= nil && Map.get(message, "topic") != nil && Map.get(message, "content") != nil) do
      true
    else
      false
    end
